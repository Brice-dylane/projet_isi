...
Future<Utilisateur> fetchUser(String login) async {
  String url = "http://192.168.1.105/webservice/index.php?action=list&map="+login;
  final response = await http.get(url);
...
Future<Data> fetchConnectionUser(String map, String search) async {
  String url = "http://192.168.1.105/webservice/index.php?action=connect&&map="+map+"&&search="+search;
  final response = await http.get(url);
  return insertFromMap(response.body).data;
}
...
Future<Data> updateUser(Utilisateur user, DateTime update, String mdp) async {
  String url = "http://192.168.1.105/webservice/index.php?action=edit&login="+user.email+"&civilite="+user.civilite+"&matricule="+user.matricule+"&cni="+user.numeroCni+"&delivrance="+user.dateDelivrance.toString()+"&expiration="+user.dateExpiration.toString()+"&tel="+user.phoneNumber+"&nom="+user.firstName+"&prenom="+user.lastName+"&sexe="+user.sexe+"&dateNais="+user.dateOfBird.toString()+"&profil="+user.authorityName+"&etablissement="+user.nomEtablissement+"&email="+user.email+"&mdp="+mdp+"&update="+update.toString();
  final response = await http.get(url);
  return insertFromMap(response.body).data;
}
...
Future<List<FormationUser>> userFormation(String login) async {
  String url = "http://192.168.1.105/webservice/index.php?action=formuser&email="+login;
  final response = await http.get(url);
...
Future<FormationState> staterepport(String login) async {
  String url = "http://192.168.1.105/webservice/index.php?action=sateform&email="+login;
  final response = await http.get(url);
